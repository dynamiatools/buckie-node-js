...
import type { Bucket, ListResult, ListFilter } from '../types/index.js'
...
export type { DownloadResult, UploadOptions, FileInfoResult }
...
export class StorageService {
  ...
  private getProvider(bucket: Bucket): StorageProvider {
    return this.registry.get(bucket.storage ?? 'local')
  }
  ...
  async validateStartup(): Promise<void> {
    ...
    for (const bucket of buckets) {
      try {
        ...
        console.warn(`[Buckie] Warning: could not validate bucket '${bucket.name}':`, err)
      }
    }
  }
  ...
  /**
   * Return file metadata without opening a read stream (used for HEAD requests).
   */
  async stat(bucket: Bucket, key: string): Promise<FileInfoResult> {
    return this.getProvider(bucket).stat(bucket, key)
  }
  ...
  async listDirectory(bucket: Bucket, keyPrefix: string, filter: ListFilter = 'all'): Promise<ListResult> {
    return this.getProvider(bucket).listDirectory(bucket, keyPrefix, filter)
  }
  ...
  async listBucket(bucket: Bucket, limit: number = 100, cursor?: string, filter: ListFilter = 'all'): Promise<ListResult> {
    return this.getProvider(bucket).listBucket(bucket, limit, cursor, filter)
  }
