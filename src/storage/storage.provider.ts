...
import { BuckieError, BuckieErrorCode } from '../errors/index.js'
...
export interface StorageProvider {
  ...
  stat(bucket: Bucket, key: string): Promise<FileInfoResult>
  ...
  listDirectory(bucket: Bucket, keyPrefix: string, filter?: ListFilter): Promise<ListResult>

  /** List all objects inside the bucket with pagination. */
  listBucket(bucket: Bucket, limit: number, cursor?: string, filter?: ListFilter): Promise<ListResult>
...
export class StorageProviderRegistry {
   ...
   * @throws {BuckieError} when no provider has been registered for `target`.
   */
  get(target: string): StorageProvider {
    const provider = this.providers.get(target)
    if (!provider) {
      throw new BuckieError(
        BuckieErrorCode.NOT_IMPLEMENTED,
