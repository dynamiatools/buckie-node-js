...
import type { Bucket, ListEntry, ListResult, ListFilter, SftpConfig } from '../types/index.js'
import { BuckieError, BuckieErrorCode, notFound } from '../errors/index.js'
import type { StorageProvider, DownloadResult, UploadOptions, FileInfoResult } from './storage.provider.js'
 ...
 * <pre>{@code
 * registry.register('sftp', new SftpStorageProvider('/var/buckie/sftp-staging'))
 * }</pre>
 */
export class SftpStorageProvider implements StorageProvider {
  ...
  private async getClient(bucket: Bucket): Promise<SftpClient> {
    ...
    try {
      ...
      throw new BuckieError(
        BuckieErrorCode.SFTP_CONNECTION_FAILED,
  ...
  private requireSftpConfig(bucket: Bucket): SftpConfig {
    if (!bucket.sftp) {
      throw new BuckieError(
        BuckieErrorCode.BUCKET_PATH_INVALID,
        `Bucket '${bucket.name}' has storage 'sftp' but no sftp config`,
        500,
      )
    }
    return bucket.sftp
  ...
  private remotePath(bucket: Bucket, key: string): string {
    ...
    if (parts.some(p => p === '..' || p === '.')) {
      throw new BuckieError(BuckieErrorCode.PATH_TRAVERSAL, 'Path traversal detected in SFTP key', 400)
    }
  ...
  async validateBucket(bucket: Bucket): Promise<void> {
    ...
    try {
      ...
      console.warn(`[Buckie] Warning: SFTP bucket '${bucket.name}' remote path '${bucket.path}' is not accessible`)
    }
  ...
  async download(bucket: Bucket, key: string): Promise<DownloadResult> {
    ...
    try {
      ...
      throw notFound(`Object '${key}' in bucket '${bucket.name}'`, BuckieErrorCode.OBJECT_NOT_FOUND)
    }

    if (remoteStat.isDirectory) {
      throw new BuckieError(BuckieErrorCode.OBJECT_IS_DIRECTORY, `'${key}' is a directory, not a file`, 400)
    }
  ...
  async stat(bucket: Bucket, key: string): Promise<FileInfoResult> {
    const remote = this.remotePath(bucket, key)
    const client = await this.getClient(bucket)
    ...
    return { size, mimeType, lastModified: new Date(modifyTime), etag }
  }
  ...
  async listDirectory(bucket: Bucket, keyPrefix: string, filter: ListFilter = 'all'): Promise<ListResult> {
    ...
    try {
      ...
      throw notFound(`Directory '${keyPrefix}' in bucket '${bucket.name}'`, BuckieErrorCode.OBJECT_NOT_FOUND)
    }
  ...
  async listBucket(bucket: Bucket, limit: number = 100, cursor?: string, filter: ListFilter = 'all'): Promise<ListResult> {
    const client = await this.getClient(bucket)
    const allEntries: ListEntry[] = []

    try {
      const entries = await client.list(bucket.path)
      for (const entry of entries) {
        const isDir = entry.type === 'd'
        if (filter === 'files' && isDir) continue
        if (filter === 'dirs' && !isDir) continue
        ...
        allEntries.push({
          key: isDir ? `${entry.name}/` : entry.name,
          size: entry.size ?? 0,
          lastModified: new Date(entry.modifyTime ?? Date.now()).toISOString(),
          isDirectory: isDir,
        })
