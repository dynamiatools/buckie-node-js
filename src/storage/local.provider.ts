...
import type { Bucket, ListResult, ListEntry, ListFilter } from '../types/index.js'
...
export class LocalStorageProvider implements StorageProvider {
  ...
  async validateBucket(bucket: Bucket): Promise<void> {
    try {
      ...
      console.warn(`[Buckie] Warning: bucket '${bucket.name}' at '${bucket.path}' is not accessible`)
      return
    }
    await this.bucketService.ensureBucketInternals(bucket.path)
  ...
  async delete(bucket: Bucket, key: string): Promise<void> {
    ...
    if (stat.isDirectory()) {
      throw new BuckieError(BuckieErrorCode.OBJECT_IS_DIRECTORY, `'${key}' is a directory and cannot be deleted`, 400)
    }
  ...
  async listDirectory(bucket: Bucket, keyPrefix: string, filter: ListFilter = 'all'): Promise<ListResult> {
    ...
    if (!stat.isDirectory()) {
      throw new BuckieError(BuckieErrorCode.OBJECT_IS_DIRECTORY, `'${keyPrefix}' is not a directory`, 404)
    }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        result.push({
          key: isDir ? `${relativeKey}/` : relativeKey,
          size: entryStat.size,
