...
program
  .name('buckie')
  .description('Buckie — filesystem-native file server with a minimal S3-style REST API')
  .version('1.0.0')
...
program
  .command('serve')
  .description('Start the Buckie server')
  .option('--host <host>', 'Host to listen on', process.env.BUCKIE_HOST ?? '0.0.0.0')
  .option('--port <port>', 'Port to listen on', process.env.BUCKIE_PORT ?? '8080')
  ...
  .action(async (name: string, bucketPath: string, opts) => {
    ...
    try {
      ...
      if (storageTarget === 'sftp') {
        if (!opts.sftpHost) {
          console.error(JSON.stringify({ ok: false, error: { code: 'BUCKIE_INVALID_ARGS', message: '--sftp-host is required for sftp storage' } }))
          process.exit(1)
        }
        if (!opts.sftpUsername) {
          console.error(JSON.stringify({ ok: false, error: { code: 'BUCKIE_INVALID_ARGS', message: '--sftp-username is required for sftp storage' } }))
          process.exit(1)
        }
        if (!opts.sftpPassword && !opts.sftpPrivateKey) {
          console.error(JSON.stringify({ ok: false, error: { code: 'BUCKIE_INVALID_ARGS', message: 'Either --sftp-password or --sftp-private-key is required for sftp storage' } }))
          process.exit(1)
        }
...
program
  ...
  .action(async (identity: string, bucket: string, opts) => {
    ...
    if (permissions.length === 0) {
      console.error(JSON.stringify({ ok: false, error: { code: 'BUCKIE_INVALID_ARGS', message: 'At least one permission (--read, --write, --delete) must be specified' } }))
      process.exit(1)
    }
...
program
  ...
  .action(async (bucketName: string, opts) => {
    const runtime = await createRuntime({ dataDir: opts.dataDir })
    try {
      const identity: string = opts.identity ?? `buckie-${randomBytes(4).toString('hex')}`
  ...
  .action((shell: string) => {
    switch (shell.toLowerCase()) {
      ...
      case 'fish':
        ...
        break
      default:
        console.error(JSON.stringify({ ok: false, error: { code: 'BUCKIE_INVALID_ARGS', message: `Unsupported shell '${shell}'. Use: bash | zsh | fish` } }))
        process.exit(1)
    }
  })
...
function printError(err: unknown): void {
  if (err && typeof err === 'object' && 'toJSON' in err && typeof (err as { toJSON: unknown }).toJSON === 'function') {
    ...
    console.error(JSON.stringify({ ok: false, error: { code: 'BUCKIE_ERROR', message: String(err) } }, null, 2))
  }
  process.exit(1)
}
