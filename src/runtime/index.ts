...
export interface BuckieRuntime {
...
export async function createRuntime(overrides?: Partial<BuckieConfig>): Promise<BuckieRuntime> {
  const config: BuckieConfig = { ...getDefaultConfig(), ...overrides }
...
export async function startServer(overrides?: Partial<BuckieConfig>): Promise<BuckieRuntime> {
  ...
  operationalLogger.log({ event: 'server_start', message: `Starting Buckie on ${config.host}:${config.port}` })
  ...
  try {
    ...
    server.log.info(`Buckie server listening on http://${config.host}:${config.port}`)
  } catch (err) {
